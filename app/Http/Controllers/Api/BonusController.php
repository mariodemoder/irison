...
class BonusController extends Controller
{
    public function forPatient(Request $request, Patient $patient): JsonResponse
    {
        ...
        $clinicId = app()->has('activeClinic') ? app('activeClinic')->id : null;
        if ($clinicId) {
    ...
    public function storeForPatient(Request $request, Patient $patient): JsonResponse
    {
        ...
        $clinicId = app()->has('activeClinic') ? app('activeClinic')->id : null;
        $bonus = $service->assignBonusToPatient($clinicId, $patient->id, $data['total_sessions'], $data['price'] ?? 0, isset($data['expires_at']) ? new \DateTime($data['expires_at']) : null);
