...
4. **Load skills by domain**: you have access to the full skill catalog. Use the `skill` tool with the matching domain name (auto-discovered from `.opencode/skills/*/SKILL.md` and vendored `.agents/skills/*/SKILL.md`) before planning or delegating. See `AGENTS.md → Skill Index` for the router. Loading the `core` skill is a good starting point for any task.
