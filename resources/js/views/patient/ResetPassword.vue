    ...
    <div class="auth-card">
      ...
      <form @submit.prevent="handleSubmit" class="auth-form">
        <div v-if="message" class="auth-success">
          {{ message }}
          <router-link :to="loginLink" class="success-link">Ir al inicio de sesión</router-link>
        </div>
...
const patientGreeting = computed(() => {
  const name = route.query.name || ''
  return name ? `Hola, ${name}` : 'Restablecer contraseña'
})
...
onMounted(() => {
  ...
  fetchClinicBranding(route.query.clinic || '')
})
...
async function handleSubmit() {
  ...
  try {
    const { data } = await patientApi.post('/auth/reset-password', {
    ...
    error.value = e?.response?.data?.message || 'Token inválido o expirado.'
  } finally {
    loading.value = false
  }
}
