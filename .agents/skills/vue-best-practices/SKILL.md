...
description: MUST be used for Vue.js tasks. Recommends Composition API with `<script setup>` as the standard approach (plain JavaScript; TypeScript only if the project already uses it). Covers Vue 3 SFCs. Load for any Vue or .vue files work. ALWAYS use Composition API unless the project explicitly requires Options API.
license: MIT
metadata:
...
- Default stack: Vue 3 + Composition API + `<script setup>` in plain JavaScript (this project does not use TypeScript; ignore `lang="ts"` in the reference examples).
...
- Prefer a feature folder layout (`components/<feature>/...`, `composables/use<Feature>.js`) when adding more than one component.
...
- Keep contracts explicit with `defineProps`/`defineEmits` (plain JS syntax). Skip TypeScript-specific contracts and `InjectionKey` typing — not used in this project.
...
- Data flow contracts are explicit.
