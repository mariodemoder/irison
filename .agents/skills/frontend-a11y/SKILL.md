...
name: frontend-a11y
description: >
  Accessibility patterns for interactive UI — semantic HTML, ARIA attributes,
  form labeling, keyboard navigation, focus management, and screen reader support.
  Examples use React/TSX but translate directly to Vue 3 components.
...
Practical accessibility patterns for interactive UI. Examples use React/TSX but apply directly to Vue 3 in this repo: translate `htmlFor` → `for`, `onClick` → `@click`, `useState`/`useEffect` → `ref`/`watch`/`computed`, inline props destructuring → `defineProps`, and the `.tsx` blocks → `.vue` `<script setup>` templates. The ARIA attributes and semantics are identical.
