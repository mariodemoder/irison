---
name: vite-patterns
description: Vite build tool patterns including config, plugins, HMR, env variables, proxy setup, SSR, library mode, dependency pre-bundling, and build optimization. Activate when working with vite.config.js/ts, Vite plugins, or Vite-based projects.
metadata:
  origin: ECC
---
...
> **This repo:** Vite **6** + Vue 3 in **plain JS** — config file is `vite.config.js` and the plugin is `@vitejs/plugin-vue` (not React). Translate the TSX examples below to JS and use `build.rollupOptions` for manual chunks (Vite 6), **not** `build.rolldownOptions` (Vite 7+ only). TypeScript-only advice (`vite-plugin-checker`, `tsc --noEmit`, `tsconfig` paths, `vite-plugin-dts`, `.tsx` imports) does not apply here.
...
> Vite 6 uses `build.rollupOptions`; `rolldownOptions` is Vite 7+ only.

```typescript
// vite.config.js — build.rollupOptions (Vite 6) / build.rolldownOptions (Vite 7+)
build: {
  rollupOptions: {
    output: {
