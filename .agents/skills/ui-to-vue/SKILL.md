...
> **This repo:** the stack is Tailwind + custom components in `resources/js/`, not Vant/Element Plus/Ant Design Vue. If you use this converter, treat `--ui` output as a draft only and re-map generated markup to the project's existing components and CSS classes (`resources/css/app.css`).
